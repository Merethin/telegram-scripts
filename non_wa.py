...
if args.activity is None:
    cursor.execute("SELECT canon_name FROM nations_dump WHERE is_wa = FALSE AND region = %s", (args.region, ))
else:
    cursor.execute(
        "SELECT canon_name FROM nations_dump WHERE is_wa = FALSE AND region = %s AND lastlogin > %s", 
...
telegrams = [
    Telegram("regional", row[0], args.tgid, args.key, client_key)
    for row in result if row[0] not in exclude
]
